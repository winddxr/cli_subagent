...
在 `py-lib/cli_subagent/profiles.py` 的 `GEMINI_PROFILE.command_template` 中添加了 `"--skip-trust"`：

```python
# py-lib/cli_subagent/profiles.py line 252-255
command_template=[
    "gemini",
    "--output-format", "json",
    "--skip-trust",  # <-- 新增
...
> 注意：`py-lib/cli_subagent/profiles.py` 第 32 行 docstring 中写的是 `"response": N`，但代码中实际使用的是 `"candidates"`。这个 **docstring 本身就是错误的**，应该修正。
...
```python
# py-lib/cli_subagent/profiles.py _normalize_codex_stats() 中添加：
return {
...
`py-lib/cli_subagent/profiles.py` 第 23-38 行 `parse_gemini_json()` 的 docstring 中记录的 JSON 结构与实际不符：
